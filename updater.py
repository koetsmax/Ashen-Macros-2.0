...
def exList(ex):
    ex_str = str(ex)
    print(
        "During the usage of updater.py, we've encountered some errors. Here's the exception:"
    )
    print(
        "======================================================================================"
    )
    ...
    print(
        "- Make sure you have installed all the dependencies. Read README.md for more info."
    )
    print(
        "If nothing works, please create a new issue on GitHub: https://github.com/jkelol111/updater.py/issues"
    )
    exit()
...
def logger(log, subcat):
    ...
    try:
        if log == "cleanup":
            ...
            if subcat == "begin":
                print(
                    infosg + " Downloading and installing updater.py for application."
                )
            elif subcat == "done":
                print(
                    infosg
                    + " Done downloading and installing updater.py for application."
                )
            elif subcat == "error":
                print(
                    warnsg + " Cannot download and install updater.py for application."
                )
            ...
            if subcat == "begin":
                ...
                print(
                    warnsg
                    + " The old configuration file (config.py) is going to be deprecated soon. Please alert your app developer if the change is not made yet!"
                )
            elif subcat == "error":
            ...
            print(
                warnsg
                + " Your OS does not support updater.py. The updater will now exit!"
            )
        elif log == "notConfigured":
            print(
                warnsg
                + " You have not configured your configuration file (config.yml). The updater will now terminate!"
            )
    except:
...
try:
    ...
    username = getuser()
    tmpBackupDirWin = str(
        "C:/Users/" + username + "/.tmp_updater" + "/" + appIdentifier
    )
