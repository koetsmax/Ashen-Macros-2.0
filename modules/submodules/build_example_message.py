...
def build_example_message(self, id_: int, status_label: tk.Label):
    ...
    if (
        ...
        self.start_button.state(["disabled"])
        status_label.config(
            text="Error! Bad Not Good to Check message!", foreground="Red"
        )
    ...
    if not "userID" in unprivate_xbox_message or not "Time" in unprivate_xbox_message:
        if unprivate_xbox_message.lower() != "delete":
            self.start_button.state(["disabled"])
            status_label.config(
                text="Error! Bad Unprivate Xbox message!", foreground="Red"
            )
    ...
    if id_ != 99:
        self.example_label = widgets.create_label(
            self.customize_window, final_string, 6, 1
        )
